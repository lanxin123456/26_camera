...
camera_forward::ActCamera::ActCamera() : 
    width_(640),
    height_(480),
    fps_(60),
    temp_frame(),
    ...
    frame_mutex_()
{
    std::string calibration_path = "src/cv/ost_a.yaml";  // 根据实际路径修改

    loadCalibrationParameters(calibration_path);
}

std::string camera_forward::ActCamera::find_lrcp_camera()
{
    glob_t glob_result;
    ...
    for(int i = 0; i < glob_result.gl_pathc; i++)
    {
        ...
        if(ioctl(fd, VIDIOC_QUERYCAP, &cap) == 0)
        {
            ...
            if(card_name.find("LRCP 1080P-60") != std::string::npos)
            {
    ...
    std::cout << "LRCP 1080P-60 forward: " << found_device << std::endl;
    return found_device;
}

//标定参数加载
bool camera_forward::ActCamera::loadCalibrationParameters(const std::string& calibration_file)
{
    try {
...
void camera_forward::ActCamera::Init()
{
    while(true)
    {
        cv::TickMeter tm;
...
void camera_forward::ActCamera::getBuffer()
{
...
void camera_forward::ActCamera::clearBuffer()
{
...
cv::Mat camera_forward::ActCamera::Update()
{
    cap.read(srcframe);
    // cv::namedWindow("srcframe", cv::WINDOW_NORMAL);
    // cv::imshow("srcframe", srcframe);
    if(!map1_.empty() && !map2_.empty() && !srcframe.empty())
    {
        cv::remap(srcframe, dstframe, map1_, map2_, cv::INTER_LINEAR);
        cv::namedWindow("dstframe", cv::WINDOW_NORMAL);
        cv::imshow("dstframe", dstframe);
    }
    else 
    {
...
void camera_forward::ActCamera::release()
{
    // std::lock_guard<std::mutex> lock_queue(queue_mutex_);
    // std::lock_guard<std::mutex> lock_frame(frame_mutex_);
    if(cap.isOpened())
    {
        cap.release();
    }
